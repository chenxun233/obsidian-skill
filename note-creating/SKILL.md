...
5. Write or rewrite the note following the rules below.
...
- Closing sections (`## Common bug`, `## Final mental model`, `## Guarantee and boundary`, `## Human-side control surface`) are top-level `##`, never `###` nested inside `## Story`.
- Never wrap a wikilink in backticks — including C++ attributes like [[likely]], [[unlikely]], [[noreturn]].
- Do not self-link the note. Use **bold** for the note's own name in prose.
