...
- Do not log edits to existing note content, memory updates such as `AGENTS.md`, or skill modifications. This includes patching, rewriting, trimming, renaming, and every other form of modification — none of them are logged.
- If a turn both creates a new note and modifies existing notes, log only the new note creation. Do not mention the modifications in the log entry — keep the supporting edits implicit.
